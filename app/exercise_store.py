...
def exercise_to_export_dict(ex: Exercise, db: Session) -> dict[str, Any]:
    ...
    return {
        ...
        "exercise": {
            ...
            "exercise_purpose": getattr(ex, "exercise_purpose", None) or "",
            "exercise_type_level_text": getattr(ex, "exercise_type_level_text", None) or "",
            "exercise_participants": getattr(ex, "exercise_participants", None) or "",
            "general_idea_text": getattr(ex, "general_idea_text", None) or "",
        ...
        "analyst_dilemma_criteria_phase_items": [
            {
                ...
                "criteria_unit_id": pi.criteria_unit_id,
                "phase_key": pi.phase_key or "",
                "flow_day_id": getattr(pi, "flow_day_id", None) or "",
                "sort_order": pi.sort_order,
                "criteria_text": pi.criteria_text or "",
                "allocated_mark": pi.allocated_mark,
                "created_at": _iso(pi.created_at),
        ...
        for row in dilemma_phase_items:
            ...
            db.add(
                AnalystDilemmaCriteriaPhaseItem(
                    ...
                    criteria_unit_id=new_cuid,
                    phase_key=str(row.get("phase_key") or "")[:32],
                    flow_day_id=str(row.get("flow_day_id") or "").strip()[:64],
                    sort_order=so,
                    criteria_text=str(row.get("criteria_text") or "")[:1000],
                    allocated_mark=mark,
                    created_at=_parse_dt(row.get("created_at")) or datetime.utcnow(),
...
def import_exercise_bundle_from_dict(db: Session, data: dict[str, Any], owner_id: int) -> int | None:
    ...
    ex = Exercise(
        ...
        exercise_purpose=str(exj.get("exercise_purpose") or ""),
        exercise_type_level_text=str(exj.get("exercise_type_level_text") or ""),
        exercise_participants=str(exj.get("exercise_participants") or ""),
        general_idea_text=str(exj.get("general_idea_text") or ""),
